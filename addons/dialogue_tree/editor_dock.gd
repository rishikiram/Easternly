...
const signal_dialogue = preload("res://addons/dialogue_tree/scripts/SignalNode.gd")
func _ready():
...
func _on_menubutton_item_pressed(id):
	...
	if id == 4:
		var signalDialogue = load("res://addons/dialogue_tree/scenes/SignalNode.tscn").instance()
		signalDialogue.connect("close_request", self, "_on_node_close", [signalDialogue])
		signalDialogue.connect("resize_request", self, "_on_node_resize", [signalDialogue])
		
		$PrimaryGraphEditor.add_child(signalDialogue, true)
	if id == 5:
		var endDialogue = load("res://addons/dialogue_tree/scenes/EndNode.tscn").instance()
		endDialogue.connect("close_request", self, "_on_node_close", [endDialogue])
		endDialogue.connect("resize_request", self, "_on_node_resize", [endDialogue])
		
		$PrimaryGraphEditor.add_child(endDialogue, true)
...
#			print("after if: ", current_node, current_node.owner)
#			didnt work, node. owner could be null
#			ResourceSaver.save(current_node.owner.filename, newRes)
			ResourceSaver.save(get_tree().edited_scene_root.filename, newRes)
...
#	print("exported Dict ", exportedDict)
	return exportedDict
...
func _on_ExportButton_button_down():
	#save function saves a very lightweight data set. just use this to export to a specific folder.
#	make_exported_dialogue()
	pass
