...
#custom
signal Dialogue_Event (event, modifiers)
...
#func _ready():
#	var dm = get_node("/root/Node/Node2D")
	
#	connects all signals to DialogManager - customm code
func connect_all_signals_to(dialog_manager_node):
	connect("Dialogue_Next", dialog_manager_node, "_on_Dialogue_Dialogue_Next")
	connect("Choice_Next", dialog_manager_node, "_on_Dialogue_Choice_Next")
	connect("Conditonal_Data_Needed", dialog_manager_node, "_on_Dialogue_Conditonal_Data_Needed")
	connect("Dialogue_Started", dialog_manager_node, "_on_Dialogue_Dialogue_Started")
	connect("Dialogue_Ended", dialog_manager_node, "_on_Dialogue_Dialogue_Ended")
#	now is connected to NPC in NPC _ready()
#	connect("Dialogue_Event", dialog_manager_node, "_on_Dialogue_Dialogue_Event")
...
func _process_next(nextDialogue, nextIndex):
	...
	if nextDialogue.has("Choices"):
		...
		queued_for_conditonal = true
		emit_signal("Conditonal_Data_Needed")
	#custom code for signal/events
	elif nextDialogue.has("Event"):
		emit_signal("Dialogue_Event", nextDialogue.Event, nextDialogue.Modifiers)
		next_dialogue()
	else:
