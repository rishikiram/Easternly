...
func _ready() -> void:
	...
	if (isLoadOnReady):
		load_data();
	#custom code to save shop price table, but it unable to save subresources#
#	var table_saver = ShopPriceTableCreator.new()
#	table_saver.save_table()
...
func parse_item_data(item_data: Dictionary) -> void:
	...
	var price = item_data.get("price", {});

	if (typeof(price) != TYPE_DICTIONARY):
		print("      Malformed json! Field 'price' is not map!");
		return;
	...
	new_item.price = Vector2(int(price.buy), int(price.sell))
