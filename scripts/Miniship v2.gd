...
#	print(PlayerVariables.sail_action)
	...
	if is_testing:
		get_node("/root/Node/CanvasLayer/Force").text = str(current_force)
		get_node("/root/Node/CanvasLayer/Speed").text = str(speed)
	#####move######
